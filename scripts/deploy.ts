...
async function deployBundler(config) {
  if (config.chainId == HARDHAT_CHAIN) {
    ...
    if (!config.bundler.url)
      throw new Error(`Bundler url is not defined for chain ${config.chainId}`);
...
export async function deployAll() {
  ...
  console.log(
    `\tDeployer balance: ${balance} (${ethers.utils.formatEther(balance)} eth)`
  );

  let config = { chainId: parseInt(chainId) };
