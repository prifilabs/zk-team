...
struct MerkleTreeData {
    ...
    uint256 rootHistorySize;  // Number of roots to save
    // The nodes of the subtrees used in the last addition of a leaf (level -> [left node, right node]).
    mapping(uint256 => uint256[2]) lastSubtrees; // Caching these values is essential to efficient appends.
    mapping(uint256 => uint256) roots; // Caching roots
}
...
library MerkleTree {
    ...
    function defaultZero(uint256 index) public pure returns (uint256) {
        ...
        revert("MerkleTree: defaultZero bad index");
    }
    ...
    function init(
        ...
        require(depth > 0 && depth <= MAX_DEPTH, "MerkleTree: tree depth must be between 1 and 32");
    ...
    function simulatedInsert(MerkleTreeData storage self, uint256 leaf) public view returns (uint256) {
        uint256 depth = self.depth;

        require(leaf < SNARK_SCALAR_FIELD, "MerkleTree: leaf must be < SNARK_SCALAR_FIELD");
        require(self.numberOfLeaves < 2**depth, "MerkleTree: tree is full");
        ...
        for (uint8 i = 0; i < depth; ) {
            if (index & 1 == 0) {
                ...
                left = self.lastSubtrees[i][0];
                right = hash;
            }
            hash = PoseidonT3.hash([left, right]);
            index >>= 1;
    ...
    function insert(MerkleTreeData storage self, uint256 leaf) public returns (uint256) {
        ...
        self.roots[self.numberOfLeaves % self.rootHistorySize] = hash;
        return hash;
    }
    ...
    */
    function isKnownRoot(MerkleTreeData storage self, uint256 _root) public view returns (bool) {
      if (_root == 0) {
        return false;
      }
      uint256 _currentRootIndex = self.numberOfLeaves % self.rootHistorySize;
    ...
    function update(
        ...
        require(newLeaf != leaf, "MerkleTree: new leaf cannot be the same as the old one");
        require(newLeaf < SNARK_SCALAR_FIELD, "MerkleTree: new leaf must be < SNARK_SCALAR_FIELD");
        require(
            verify(self, leaf, proofSiblings, proofPathIndices),
            "MerkleTree: leaf is not part of the tree"
        );
        ...
        require(updateIndex < self.numberOfLeaves, "MerkleTree: leaf index out of range");
        
        uint256 index = self.numberOfLeaves % self.rootHistorySize;
        for (uint8 i = 0; i < self.rootHistorySize; ) {
            if (i == index) self.roots[i] = hash;
            else self.roots[i] = 0;
            unchecked {
                ++i;
            }
    ...
    function verify(
        ...
        require(
            ...
            "MerkleTree: length of path is not correct"
        );
        ...
        for (uint8 i = 0; i < depth; ) {
            require(
                ...
                "MerkleTree: sibling node must be < SNARK_SCALAR_FIELD"
            );

            require(
                proofPathIndices[i] == 1 || proofPathIndices[i] == 0,
                "MerkleTree: path index is neither 0 nor 1"
            );
        ...
        return hash == self.roots[self.numberOfLeaves % self.rootHistorySize];
    }
}